...
* [HKDF](https://en.wikipedia.org/wiki/HKDF): HKDF is a [key derivation function](https://en.wikipedia.org/wiki/Key_derivation_function) (which basically takes in some paramteres and returns a cryptographically secure key, other examples are [PBKDF2](https://en.wikipedia.org/wiki/PBKDF2) and [Argon2](https://en.wikipedia.org/wiki/Argon2)) with some [HMAC](https://en.wikipedia.org/wiki/HMAC) sprinkled on top of it, to ensure integry and authenticity. It's going to be the main function I'll use for the Double Ratchet implementation, since it is esentually a ratchet function.
...
First we set up some interfaces for TypeScript, if you're implementing this in pure JavaScript, you can skip this step. The `setupX3DH` function starts off easy: we generate the identity key and prekey. They're both using the P-256 curve with key usages set to `deriveKey`. Note that `setupX3DH` is an async function, since we'll need to use `await` for `SubtleCrypto`'s functions.

<Warning>Indented lines are part of a function</Warning>

```typescript
    const identitySigningKey = await crypto.importKey(
        "jwk", await crypto.exportKey("jwk", identityKeyPair.privateKey),
